<div id="kanban-wrapper">
    ...
    foreach ($clients as $client) {
        ...
        $exising_items = get_array_value($columns_data, $client->lead_status_id);
        if (!$exising_items) {
            $exising_items = "";
        }

        $source = "";
        if ($client->lead_source_title) {
            $source = "<div class='float-start'><i data-feather='anchor' class='icon-14 text-off mr5'></i> " . $client->lead_source_title . "</div>";
        }

        $owner = "";
        if ($client->owner_id) {
            $owner = "<div class='float-end'><span class='avatar float-none' data-bs-toggle='tooltip' title='" . $client->owner_name . "'><img src='" . get_avatar($client->owner_avatar) . "' class='me-0'></span></div>";
        }

        $client_labels = "";
        $client_labels_data = make_labels_view_data($client->labels_list);
        if ($client_labels_data) {
            $client_labels .= "<div class='meta mr5'>$client_labels_data</div>";
        }

        $clients_total_counts = "<div class='mt10 float-end'>";

        if (!$client_labels) {
            $clients_total_counts = "<div class='float-start'>";
        }

        //total contacts
        if ($client->total_contacts_count) {
            $clients_total_counts .= "<span class='mr5' title='" . app_lang("contacts") . "'><i data-feather='users' class='icon-14 text-off'></i> " . $client->total_contacts_count . "</span> ";
        }

        //total events
        if ($client->total_events_count) {
            $clients_total_counts .= "<span class='mr5' title='" . app_lang("events") . "'><i data-feather='calendar' class='icon-14 text-off'></i> " . $client->total_events_count . "</span> ";
        }

        //total notes
        if ($client->total_notes_count) {
            $clients_total_counts .= "<span class='mr5' title='" . app_lang("notes") . "'><i data-feather='book' class='icon-14 text-off'></i> " . $client->total_notes_count . "</span> ";
        }

        //total estimates
        if ($client->total_estimates_count) {
            $clients_total_counts .= "<span class='mr5' title='" . app_lang("estimates") . "'><i data-feather='file' class='icon-14 text-off'></i> " . $client->total_estimates_count . "</span> ";
        }

        //total estimate requests
        if ($client->total_estimate_requests_count) {
            $clients_total_counts .= "<span class='mr5' title='" . app_lang("estimate_requests") . "'><i data-feather='file' class='icon-14 text-off'></i> " . $client->total_estimate_requests_count . "</span> ";
        }

        //total files
        if ($client->total_files_count) {
            $clients_total_counts .= "<span class='mr5' title='" . app_lang("files") . "'><i data-feather='file-text' class='icon-14 text-off'></i> " . $client->total_files_count . "</span> ";
        }

        $clients_total_counts .= "</div>";

        $open_in_new_tab = anchor(get_uri("clients/view/" . $client->id), "<i data-feather='external-link' class='icon-14'></i>", array("target" => "_blank", "class" => "float-end", "title" => app_lang("details")));

        //custom fields to show in kanban
        $kanban_custom_fields_data = "";
        $kanban_custom_fields = get_custom_variables_data("clients", $client->id, $login_user->is_admin);
        ...
        $item = $exising_items . "<span class='client-kanban-item kanban-item' data-id='$client->id' data-sort='$client->new_sort' data-post-id='$client->id'>
                    <div class='selection-pe-none'><span class='avatar'><img src='" . get_avatar($client->primary_contact_avatar) . "'></span>" . anchor(get_uri("clients/view/" . $client->id), $client->company_name) . $open_in_new_tab . "</div><div class='clearfix'></div>" .
                "<div class='mt15'>" . $source . $owner . "</div>" . $kanban_custom_fields_data . "<div class='clearfix'></div>" .
                $clients_total_counts . $client_labels . "</span>";

        $columns_data[$client->lead_status_id] = $item;
    }
    ?>
            ...
            <li class="kanban-col" >
                ...
                <div class="kanban-input general-form hide">
                    ...
                    echo form_input(array(
                        ...
                        "value" => "",
                        "class" => "form-control",
                        "placeholder" => app_lang('add_a_client')
                    ));
                    ?>
                </div>
...
<script type="text/javascript">
    ...
    saveStatusAndSort = function ($item, status) {
        ...
        $.ajax({
            url: '<?php echo_uri("clients/save_client_sort_and_status") ?>',
            type: "POST",
