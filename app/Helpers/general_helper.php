...
if (!function_exists('prepare_estimate_request_pdf')) {
    function prepare_estimate_request_pdf($estimate_request_data, $mode = "download", $is_mobile_preview = false) {
        $pdf = new Pdf("estimate_request");
        ...
        if (!$estimate_request_data || !is_array($estimate_request_data)) {
            log_message('error', 'Invalid or empty estimate_request_data in prepare_estimate_request_pdf');
            throw new \Exception('Invalid estimate request data');
        }

        // Log data for debugging
        log_message('debug', 'Preparing PDF with data: ' . print_r($estimate_request_data, true));

        $estimate_request_data["mode"] = clean_data($mode);
        $html = view("estimate_requests/estimate_request_pdf", $estimate_request_data);
        ...
        $estimate_request_info = get_array_value($estimate_request_data, "estimate_request_info");
        $pdf_file_name = "estimate_request_" . $estimate_request_info->id . ".pdf";
        ...
        if ($mode === "download") {
            ...
            if ($is_mobile_preview) {
                ...
                $pdf_content = $pdf->Output($pdf_file_name, "S");
...
if (!function_exists('get_invoice_status_label')) {
    ...
    function get_invoice_status_label($invoice_info, $return_html = true) {
        ...
        $invoice_status = "<span class='mt0 badge $invoice_status_class'>" . app_lang($status) . "</span>";
        if ($return_html) {
...
if (!function_exists('get_contract_making_data')) {
    function get_contract_making_data($contract_id) {
        ...
        log_message('debug', 'Fetching contract making data for Contract ID: ' . $contract_id);
        
        $contract_info = $ci->Contracts_model->get_details(array("id" => $contract_id))->getRow();
        if (!$contract_info) {
            log_message('error', 'No contract found for Contract ID: ' . $contract_id);
            return null;
        }

        $data['contract_info'] = $contract_info;
        $client_id = $contract_info->client_id;
        log_message('debug', 'Client ID for Contract ID ' . $contract_id . ': ' . $client_id);
        
        // Fetch client info with client groups
        $client_info = $ci->Clients_model->get_details(array("id" => $client_id))->getRow();
        if (!$client_info || !$client_info->id) {
            log_message('error', 'No client found for Client ID: ' . $client_id);
        } else {
            log_message('debug', 'Client Details for Client ID ' . $client_id . ': ' . print_r([
                'id' => $client_info->id,
                'company_name' => $client_info->company_name,
                'is_lead' => $client_info->is_lead,
                'client_groups' => $client_info->client_groups
            ], true));
        }
        
        // Assign client groups as 'region'
        ...
        // Fetch contract custom fields
        $data['contract_info']->custom_fields = $ci->Custom_field_values_model->get_details(array(
            "related_to_type" => "contracts",
            "show_in_contract" => true,
            "related_to_id" => $contract_id
        ))->getResult();
        ...
        $client_custom_fields = $ci->Custom_field_values_model->get_details($options)->getResult();

        // Debug: Log client custom fields
        $debug_data = [];
        foreach ($client_custom_fields as $field) {
            $debug_data[] = [
                'id' => isset($field->id) ? $field->id : 'N/A',
                'custom_field_id' => isset($field->custom_field_id) ? $field->custom_field_id : 'N/A',
                'value' => isset($field->value) ? $field->value : 'N/A',
                'custom_field_title' => isset($field->custom_field_title) ? $field->custom_field_title : 'N/A',
                'custom_field_type' => isset($field->custom_field_type) ? $field->custom_field_type : 'N/A'
            ];
        }
        log_message('debug', 'Client Custom Fields for Contract ID ' . $contract_id . ', Client ID ' . $client_id . ': ' . print_r($debug_data, true));

        $data['client_info']->custom_fields = $client_custom_fields;
        return $data;
    }
}
...
if (!function_exists('prepare_contract_view')) {
    function prepare_contract_view($contract_data) {
        if ($contract_data) {
            ...
            log_message('debug', 'Contract Content for Contract ID ' . $contract_info->id . ': ' . $contract_info->content);
            ...
            if ($contract_info->status === "accepted" && ($signer_info || $contract_info->accepted_by)) {
                ...
                $parser_data["CLIENT_SIGNING_DATE"] = get_array_value($signer_info, "signed_date") ? format_to_relative_time(get_array_value($signer_info, "signed_date")) : "";
            ...
            if ($contract_info->staff_signed_by) {
                ...
                $parser_data["STAFF_SIGNING_DATE"] = get_array_value($signer_info, "staff_signed_date") ? format_to_relative_time(get_array_value($signer_info, "staff_signed_date")) : "";
            ...
            $parser_data["COMPANY_RATE"] = $company_info->website;
            ...
            // Define mapping of client custom field IDs to user-friendly variable names
            $client_custom_field_mappings = [
                '164' => 'company_name',
                '240' => 'source',
                '234' => 'volume',
                '237' => 'competitor',
                '167' => 'estimated_close_date',
                '168' => 'distance_from_bulk_plant',
                '232' => 'account_number',
                '236' => 'additional_details',
            ];

            // Parse contract custom fields
            if (isset($contract_info->custom_fields) && $contract_info->custom_fields) {
                foreach ($contract_info->custom_fields as $field) {
                    ...
                    $parser_data[$variable] = $field->value ? view("custom_fields/output_" . $field->custom_field_type, array("value" => $field->value)) : "";
                    log_message('debug', 'Contract Custom Field Variable: ' . $variable . ' => Value: ' . ($parser_data[$variable] ?: 'Empty'));
                }
            }

            // Parse client custom fields
            if (isset($client_info->custom_fields) && $client_info->custom_fields) {
                foreach ($client_info->custom_fields as $field) {
                    $field_id = isset($field->custom_field_id) ? $field->custom_field_id : null;
                    if ($field_id === null) {
                        log_message('error', 'Missing custom_field_id for client custom field: ' . print_r($field, true));
                        continue;
                    }
                    // Support existing CF_client_ placeholders
                    $variable = "CF_client_" . $field_id;
                    $value = isset($field->value) ? $field->value : "";
                    $field_type = isset($field->custom_field_type) ? $field->custom_field_type : "text";
                    $parser_data[$variable] = $value ? view("custom_fields/output_" . $field_type, array("value" => $value)) : "N/A";
                    log_message('debug', 'Client Custom Field Variable: ' . $variable . ' => Value: ' . ($parser_data[$variable] ?: 'N/A'));
                    // Support titled placeholders
                    $titled_variable = "CF_client_" . $field_id . "_" . str_replace(' ', '_', strtolower(trim($field->custom_field_title)));
                    $parser_data[$titled_variable] = $parser_data[$variable];
                    log_message('debug', 'Client Custom Field Titled Variable: ' . $titled_variable . ' => Value: ' . ($parser_data[$titled_variable] ?: 'N/A'));
                    // Map to new variable names
                    if (isset($client_custom_field_mappings[$field_id])) {
                        $new_variable = $client_custom_field_mappings[$field_id];
                        $parser_data[$new_variable] = $value ? view("custom_fields/output_" . $field_type, array("value" => $value)) : "N/A";
                        log_message('debug', 'New Client Custom Field Variable: ' . $new_variable . ' => Value: ' . ($parser_data[$new_variable] ?: 'N/A'));
                    }
                }
            } else {
                log_message('debug', 'No client custom fields available for Client ID ' . $client_info->id);
            }

            // Assign values for missing fields
            foreach ($client_custom_field_mappings as $field_id => $variable) {
                if (!isset($parser_data[$variable])) {
                    $parser_data[$variable] = "N/A";
                    log_message('debug', 'Assigned Missing Client Custom Field Variable: ' . $variable . ' => Value: N/A');
                }
            }

            // Debug: Log all parser data
            log_message('debug', 'Parser Data for Contract ID ' . $contract_info->id . ': ' . print_r($parser_data, true));

            $parser = \Config\Services::parser();
            $content = remove_custom_field_titles_from_variables($contract_info->content);
            log_message('debug', 'Contract Content Before Parsing: ' . $content);
            $contract_view = $parser->setData($parser_data)->renderString($content);
            log_message('debug', 'Contract Content After Parsing: ' . $contract_view);
...
if (!function_exists('remove_custom_field_titles_from_variables')) {
    ...
    function remove_custom_field_titles_from_variables($content) {
        ...
        log_message('debug', 'Original Contract Content for Placeholder Processing: ' . $content);

        // Match only CF_ and CF_client_ prefixed variables, exclude new variables like {company_name}
        preg_match_all('#\{(CF_|CF_client_)(.*?)\}#', $content, $matches);
        $custom_fields = get_array_value($matches, 2);
        $prefixes = get_array_value($matches, 1);
        ...
        if ($custom_fields && count($custom_fields)) {
            foreach ($custom_fields as $index => $custom_field) {
                ...
                $custom_field_id = get_array_value($explode_custom_field, 0);
                $custom_field_new_variables[] = "{" . $prefix . $custom_field_id . "}";
            }
        }

        $new_content = $content;
        if ($custom_field_new_variables) {
            $new_content = preg_replace_callback('#\{(CF_|CF_client_)(.*?)\}#', function ($matches) use (&$custom_field_new_variables) {
                return array_shift($custom_field_new_variables);
            }, $content);
        }

        // Log processed content
        log_message('debug', 'Content After Removing Custom Field Titles: ' . $new_content);
...
if (!function_exists('get_available_contract_variables')) {
    function get_available_contract_variables() {
        $variables = array(
            ...
            "region" // Add region variable
        );

        $ci = new Security_Controller(false);

        // Prepare contract custom fields
        $contract_custom_fields = $ci->Custom_fields_model->get_combined_details("contracts", 0, $ci->login_user->is_admin, $ci->login_user->user_type)->getResult();
        if ($contract_custom_fields) {
            foreach ($contract_custom_fields as $custom_field) {
        ...
        // Add mapped client custom fields to variables
        foreach ($client_custom_field_mappings as $variable_name) {
            array_push($variables, $variable_name);
        }

        log_message('debug', 'Available Contract Variables: ' . print_r($variables, true));
...
if (!function_exists('get_subscription_status_label')) {
    ...
    function get_subscription_status_label($subscription_info, $return_html = true) {
        ...
        $subscription_status = "<span class='mt0 badge $subscription_status_class'>" . app_lang($status) . "</span>";
        if ($return_html) {
...
if (!function_exists('get_estimate_status_label')) {
    ...
    function get_estimate_status_label($estimate_info, $return_html = true) {
        ...
        $estimate_status = "<span class='mt0 badge $estimate_status_class'>" . app_lang($estimate_info->status) . "</span>";
        if ($return_html) {
...
if (!function_exists('prepare_contract_pdf')) {
    ...
    function prepare_contract_pdf($contract_data, $mode = "download") {
        ...
        if ($contract_data) {
            ...
            // Debug: Log HTML content before writing to PDF
            log_message('debug', 'Contract PDF HTML Content for Contract ID ' . get_array_value($contract_data, "contract_info")->id . ': ' . $html);
...
if (!function_exists('add_client_note')) {
    /**
     * Add a note to a client or lead profile
     * 
     * @param int $related_id The ID of the client or lead
     * @param string $related_type 'client' or 'lead'
     * @param string $description Description of the action performed
     * @param int $created_by User ID of the person performing the action (optional)
     * @return int|bool The ID of the saved note or false on failure
     */
    function add_client_note($related_id, $related_type, $description, $created_by = 0) {
        $ci = new Security_Controller(false); // Access login user info
        
        $Notes_model = model('App\Models\Notes_model');
        
        $note_data = array(
            "description" => $description,
            "created_by" => $created_by ?: (isset($ci->login_user->id) ? $ci->login_user->id : 0),
            "created_at" => get_current_utc_time(),
            "title" => "Automated Action Note", // Default title
            "is_public" => 0 // Private by default
        );
        
        // Set client_id (used for both clients and leads, as per clients table with is_lead flag)
        if ($related_type === 'client' || $related_type === 'lead') {
            $note_data['client_id'] = $related_id;
        } else {
            return false; // Invalid related_type
        }
        
        $note_data = clean_data($note_data); // Clean data for security
        $note_id = $Notes_model->ci_save($note_data);
        
        if ($note_id) {
            // Log a notification
            log_notification("note_added", array(
                "note_id" => $note_id,
