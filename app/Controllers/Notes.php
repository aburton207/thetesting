...
class Notes extends Security_Controller {
   ...
   private function _make_row($data) {
    ...
    // Get the author's name (assuming Notes_model joins with users table or similar)
    $author = $data->created_by_user_name ? $data->created_by_user_name : "-";
    ...
    return array(
        $data->created_at,              // 0: Raw created_at for sorting
        format_to_relative_time($data->created_at), // 1: Relative time
        $title,                         // 2: Title with labels
        $data->category_name ? $data->category_name : "-", // 3: Category
        $author,                        // 4: Author (new)
        $description,                   // 5: Description (new)
        $file_download_link . $files_link, // 6: Files
        $actions                        // 7: Actions
    );
}
