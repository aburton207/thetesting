  ...
  function modal_form() {
   ...
   function save() {
    ...
    if ($this->login_user->is_admin || get_array_value($this->login_user->permissions, "client") === "all") {
        // User has access to change owner_id
        $posted_owner_id = $this->request->getPost('owner_id');
        if ($posted_owner_id) {
            $data["owner_id"] = $posted_owner_id;
        }
    } else if (!$client_id) {
        // For new clients, set owner_id to the logged-in user's ID if not already set
...
function list_data() {
    ...
    // Handle server-side or client-side processing
...
function view($client_id = 0, $tab = "", $folder_id = 0) {
    ...
    if ($client_id) {
        ...
        if ($client_info && !$client_info->is_lead) {
            ...
            if (!$view_data['owner_name'] || $view_data['owner_name'] === "-") {
                $owner = $this->Users_model->get_one($client_info->owner_id);
                $view_data['owner_name'] = $owner->id ? $owner->first_name . " " . $owner->last_name : "-";
            }

            // Debug owner_id
            log_message('debug', 'View Client ID: ' . $client_id . ', Owner ID: ' . $client_info->owner_id . ', Owner Name: ' . $view_data['owner_name']);
            ...
            // Even if hidden, admin can view all information of client
...
function save_client_status($id = 0) {
    validate_numeric_value($id);
    $this->_validate_client_manage_access($id);
...
private function _get_headers_for_import() {
    return array(
       ...
       array("name" => "contact_email"),
        ...
        array("name" => "owner_name", "custom_validation" => function ($owner_name, $row_data) {
            if ($owner_name) {
                $owner = $this->Users_model->get_one_where(array(
                    "CONCAT(first_name, ' ', last_name)" => trim($owner_name),
                    "user_type" => "staff",
                    "deleted" => 0
                ));
                if (!$owner->id) {
                    return array(
                        "error" => app_lang("import_error_invalid_owner_name"),
                        "highlight" => true
                    );
                }
            }
        ...
        array("name" => "lead_status_id", "custom_validation" => function ($lead_status_id, $row_data) {
            if ($lead_status_id) {
                if (!is_numeric($lead_status_id)) {
                    return array(
                        "error" => app_lang("import_error_invalid_lead_status_id_numeric"),
                        "highlight" => true
                    );
                }
                $status = $this->Lead_status_model->get_one($lead_status_id);
                if (!$status->id) {
                    return array(
                        "error" => app_lang("import_error_invalid_lead_status_id"),
                        "highlight" => true
                    );
                }
            }
...
private function _init_required_data_before_starting_import() {
    ...
    // Initialize staff members mapping
    $staff_members = $this->Users_model->get_details(array("user_type" => "staff", "deleted" => 0))->getResult();
    $this->staff_id_by_name = array();
    foreach ($staff_members as $staff) {
        $full_name = trim($staff->first_name . " " . $staff->last_name);
        $this->staff_id_by_name[$full_name] = $staff->id;
    }
}
...
private function _save_a_row_of_excel_data($row_data) {
 ...
 private function _prepare_client_data($row_data) {
    ...
    foreach ($row_data as $column_index => $value) {
        ...
        } else if ($column_name == "owner_name") {
            $owner_name = trim($value);
            $owner_id = get_array_value($this->staff_id_by_name, $owner_name);
            if ($owner_id) {
    ...
    function clients_volume_by_source_chart() {
        ...
        $rows = $this->Clients_model->get_volume_by_source($options);

        $labels = $volume = array();
        foreach ($rows as $row) {
            $labels[] = $row->source_title ? $row->source_title : app_lang('unknown');
            $volume[] = floatval($row->volume);
        }
