...
private function _make_estimate_request_row($data) {
    ...
    if ($data->company_name) {
        ...
        // Fetch custom field values for IDs 246, 267, 268, 269, 270, 271, 275 and 281-288
        $custom_field_246 = "";
        $custom_field_267 = "";
        $custom_field_268 = "";
        $custom_field_269 = "";
        ...
        $custom_field_values = $this->Custom_field_values_model->get_details(array(
            ...
            "custom_field_id" => array(246, 267, 268, 269, 270, 271, 275, 281, 282, 283, 284, 285, 286, 287, 288)
        ))->getResult();

    foreach ($custom_field_values as $field) {
        if ($field->custom_field_id == 246) {
            $custom_field_246 = $this->template->view("custom_fields/output_" . $field->custom_field_type, array("value" => $field->value));
        } elseif ($field->custom_field_id == 267) {
            $custom_field_267 = $this->template->view("custom_fields/output_" . $field->custom_field_type, array("value" => $field->value));
        } elseif ($field->custom_field_id == 268) {
            $custom_field_268 = $this->template->view("custom_fields/output_" . $field->custom_field_type, array("value" => $field->value));
        } elseif ($field->custom_field_id == 269) {
        ...
        } elseif ($field->custom_field_id == 288) {
            $custom_field_288 = $this->template->view("custom_fields/output_" . $field->custom_field_type, array("value" => $field->value));
        }
    }
    ...
    $download_pdf = '<li role="presentation"><a href="' . get_uri("estimate_requests/download_pdf/" . $data->id) . '" class="dropdown-item"><i data-feather="download" class="icon-16"></i> ' . app_lang('download_pdf') . '</a></li>';
    ...
    // Only show download option for staff (consistent with view_estimate_request logic)
    $options = '<span class="dropdown inline-block">
                    <button class="btn btn-default dropdown-toggle caret mt0 mb0" type="button" data-bs-toggle="dropdown" aria-expanded="true" data-bs-display="static">
                        <i data-feather="tool" class="icon-16"></i>
                    </button>
                    <ul class="dropdown-menu dropdown-menu-end" role="menu">' . $edit . $request_status . $add_estimate;
    if ($this->login_user->user_type == "staff") {
        $options .= $download_pdf;
    }
    $options .= $delete . '</ul>
                </span>';
  ...
  // Download PDF for an estimate request
    function download_pdf($estimate_request_id = 0) {
        ...
        // Check permissions (reuse existing logic for view access)
